...
func Test_InitArrayList(t *testing.T) {
	a := -1
	lst, err := InitArrayList(a)
	assert.Equal(t, nil, lst)
	...
	assert.Equal(t, 0, lst.(*ArrayList).Length())
	assert.Equal(t, a, lst.(*ArrayList).Size())
}
...
func Test_IsFull(t *testing.T) {
	...
	lst.Insert(0, 1)
	lst.Insert(0, 1)
	assert.Equal(t, true, lst.IsFull())
}

...
func Test_InitCircleLinkList(t *testing.T) {
	lst, err := InitCircleLinkList()
	assert.NotEqual(t, nil, lst)
	assert.Equal(t, nil, err)

	assert.Equal(t, 0, lst.(*CircleLinkList).Length())
	assert.Equal(t, 0, lst.(*CircleLinkList).Size())
}
